...
public class MicDemo {
      ...
      while (System.in.available() == 0) {
        if (recorder != null) {
          ...
          if (verbose) {
            ...
            System.out.format(
                "%14s | %5s | %5s | %10s | %11s\n",
                "word", "start", "end", "confidence", "speaker tag");
      ...
      try {
        ...
        if (audioDeviceIndex < 0) {
          throw new IllegalArgumentException(
              String.format(
                  "Audio device index %s is not a " + "valid positive integer.",
                  audioDeviceIndexStr));
        }
      } catch (Exception e) {
        throw new IllegalArgumentException(
  ...
  private static Options buildCommandLineOptions() {
    ...
    options.addOption(
        Option.builder("v").longOpt("verbose").desc("Enable verbose logging.").build());

    options.addOption(
        new Option("sd", "show_audio_devices", false, "Print available recording devices."));
